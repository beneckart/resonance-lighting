...
- [ ] **Firmware guard: input re-qualification kick (bright-sun connect latch)** — 2026-06-11: connecting/booting with the panel at full-sun Voc (~6.0-6.2 V) latches the BQ input fault (`sgood=0`, zero draw) and only FULL VBUS removal clears it (shade to 4.7 V did not). Production firmware should detect supply-present-but-not-good (sv > ~4 V, sgood=0, sustained) and kick re-qualification (charger re-init / HIZ toggle via SDK — investigate the right register path). Also feeds panel selection: production panel's COLD-morning Voc must clear the charger input window (Ben).
