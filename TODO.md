...
- [ ] Repeat low-battery maintenance-AP OTA boundary test with a second host network
  interface (USB Ethernet/phone tether) or direct USB/external power. First attempt
  2026-06-29: at ~2.57 V the peer entered/attempted maint-AP, then brownout-reset and
  emitted only two heartbeats at ~2.33 V before going stale; single-laptop-WiFi testing
  also strands the Codex backend when joined to the peer AP (Ben).
...
- [ ] **Firmware fix: VBUS_OVP=1 + HIZ-toggle re-qualification kick (bright-sun connect latch) — ROOT-CAUSED 2026-06-12 from the BQ25628E datasheet (SLUSFA4C).** The input-OVP gate is a REGISTER BIT: VBUS_OVP=0 (POR default) = 6.1/6.4/6.7 V rising threshold — our 6.15 V hot-Voc connect tripped a min-spec part; VBUS_OVP=1 = 18.2/18.5/18.8 V. Input qualification is EDGE-triggered (runs only on an insertion edge — why shade-to-4.7 V didn't recover but full unplug did). Fix in production + bench firmware: (1) set VBUS_OVP=1 at init (register address in datasheet 8.6 reg map; SDK doesn't expose it — raw I2C write to the BQ at 0x6A), (2) supply-present-but-not-good watchdog -> EN_HIZ toggle to synthesize a fresh qualification edge. Together these UNBLOCK the standard "6V" panel class (Voc 6.8-7.4) for procurement — panel spec becomes Vmp(STC) >= ~5.4 V, Voc <= ~16 V, through the sweep harness. PREREQUISITE for any panel buy with Voc > 6 V (Ben).
