...
- [x] **Outdoor solar telemetry over ESP-NOW + WiFi range diagnostic — BOTH DONE 2026-06-08** (LOG cont. 7/8/9): plan `docs/tests/SOLAR_TELEMETRY_RANGE_PLAN_2026-06-08.md`. Next: a **sizing-oriented** solar run (below).
  ...
  - [ ] **NEXT: sizing-oriented solar run** — now that supply telemetry works, run a **representative** harvest+budget test to actually spec cell (LFP mAh) + panel (W): (1) realistic **sleep/low-duty load** (NOT the always-on radio+scan stress firmware — those bench `load_w` numbers are worst-case/conservative; field sleeps during sunshine), (2) **harvest across sun/cloud/shade + panel orientation** (it's very orientation-sensitive), (3) a full charge/discharge to anchor SOC + map buck-boost efficiency vs VBAT on LFP. Pair with the bottom-up nightly-budget re-derivation (Field reliability TODO) (Ben).
