...
- [x] **Outdoor solar telemetry over ESP-NOW + WiFi range diagnostic — BOTH DONE 2026-06-08** (LOG cont. 7/8/9): plan `docs/tests/SOLAR_TELEMETRY_RANGE_PLAN_2026-06-08.md`. Next: a **sizing-oriented** solar run (below).
  ...
  - [~] **Sizing-oriented solar campaign** — in progress 2026-06-08 (LOG cont. 10). Spec cell (LFP mAh) + panel (W) from harvest (Wh/day at MPP) vs load (sleep + LED show). Progress + open items:
    ...
    - [~] **LFP drawdown** (always-on, battery-only) running overnight → discharge curve, gauge accuracy, delivered capacity, cutoff. Started from ~SOC 60–76 % (NOT full — a 0–100 % capacity run needs a USB top-up first) (Ben).
    - [ ] **Measure the sleep-cycle DUTY-CYCLED average** (battery-only, multi-hour) — the low overnight/idle budget number; still un-measured (the drawdown used always-on, not sleep) (Ben).
