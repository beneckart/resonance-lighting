...
- [x] **Outdoor solar telemetry over ESP-NOW + WiFi range diagnostic — BOTH DONE 2026-06-08** (LOG cont. 7/8/9): plan `docs/tests/SOLAR_TELEMETRY_RANGE_PLAN_2026-06-08.md`. Next: a **sizing-oriented** solar run (below).
  ...
  - [~] **Sizing-oriented solar campaign** — in progress 2026-06-08 (LOG cont. 10). Spec cell (LFP mAh) + panel (W) from harvest (Wh/day at MPP) vs load (sleep + LED show). Progress + open items:
    ...
    - [~] **Clean full-sun MPP sweep** — TOOLING READY 2026-06-10, **fully wireless** (LOG cont. + cont. 2), awaiting the TSL2591/SHT31 arrival (~06-11) + a stable-sun day. The 06-08 sweep was cloud/temp-confounded (magnitude TBD). Built: net_bench fw 2026-06-10.1 (master `m<v10>` explicit setpoint; peer auto-probes TSL2591 lux + SHT31 panel-temp on STEMMA and appends `lux=/ptc=/btc=` to the heartbeat — no outdoor laptop/PAR tether; reflash BOTH boards: master over USB, peer OTA), `ops/bench/mpp_sweep.py` (anchor-revisit drift detection, 3x re-send of the unacked broadcast, saturation/instability/dark-panel flags, IR-gun prompts as spot-check, restores 5.5 V on exit), `ops/bench/mpp_analyze.py` (light-normalized P-vs-VINDPM, anchor drift, best-vs-5.5 ratio, Vmp shift). Host side validated end-to-end vs a simulated master; **sensor path needs on-hw validation when modules arrive**. **Run procedure:** (1) chain TSL2591+SHT31 on the peer's STEMMA, tape SHT31 to the panel BACK; (2) precondition SOC <~60% so the charger stays in bulk/CC; (3) indoor dry run at a sunny window (validates sensors + command path; optional Apogee cross-check via `--par-port`) — NO setpoints > supply on USB; (4) cool-AM + hot-midday sessions, panel flat, `--session cool-am` / `hot-noon`; (5) `mpp_analyze.py` both files. If `lux=sat` in full sun, add a paper/PTFE diffuser over the TSL2591 (relative use survives it) (Ben).
