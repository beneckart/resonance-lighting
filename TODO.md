...
- [ ] **Revisit 8-bit LED low-end dimming for the ambient look** (deferred 2026-06-07): WS2812/SK6812 are 8-bit/channel, so gamma-on dims to OFF below ~brightness 24 (the ambient "~10%" spec sits in this dead-zone); gamma-off gives ultra-dim but non-linear steps. Options: dim-floor `max(1,gamma8(x))`, gentler gamma, gamma-on-color-only, or temporal dithering. See LOG 2026-06-07 + `firmware/POWERFEATHER_NOTES.md` (Ben). 2026-08-18: the dim-floor table now exists in-tree as `firmware/fixture/src/core/gamma.h` (`resGamma8`, salvaged from the retired `codex/commissioning-mode` line); deliberately NOT wired into the render path -- commissioning renders stay direct linear per `led_driver.cpp`. Wire it behind a show-mode flag after a visual check (Ben/Codex).
