...
- [ ] **Outdoor solar telemetry over ESP-NOW + WiFi range diagnostic** — handoff plan written: `docs/tests/SOLAR_TELEMETRY_RANGE_PLAN_2026-06-08.md`. (a) bridge power telemetry over ESP-NOW so solar logs from the yard without WiFi-STA (ESP32 is 2.4GHz-only; can't hold the Eero from outside); (b) quick WiFi RSSI/BSSID/scan diagnostic to confirm the 2.4GHz/roaming cause + place the field maintenance AP (fresh agent).
