#!/usr/bin/env bash
# Build (and optionally flash) the Solenoid Demo (3 V mini solenoid strike bench).
#
# Usage:
#   ./build.sh                       # compile only
#   ./build.sh --port /dev/ttyACM1   # compile + USB flash
#   ./build.sh --ota 192.168.4.xx    # compile + WiFi OTA to a running solenoid_demo
#   ./build.sh --pin 16 --port ...   # override gate pin (default 10 / A0)
#
# WiFi: the sketch #includes wifi_secrets.h. If it's missing, build.sh copies the
# one from ../speaker_demo (falling back to ../sway_demo, ../led_studio,
# ../power_bench). With no secrets at all, the firmware serves a SoftAP
# "ResonanceSolenoid" (pw resonance) at 192.168.4.1.
set -euo pipefail
...
if [[ ! -f "${SKETCH_DIR}/wifi_secrets.h" ]]; then
  for src in ../speaker_demo ../sway_demo ../led_studio ../power_bench; do
...
if [[ -n "${PORT}" ]]; then
  ...
  echo "done. open the serial monitor (115200) to see the IP, or http://solenoiddemo.local/"
fi

if [[ -n "${OTA_IP}" ]]; then
  BIN="${SKETCH_DIR}/build/esp32.esp32.esp32s3_powerfeather/solenoid_demo.ino.bin"
