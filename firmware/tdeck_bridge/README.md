...
118-fixture physical roster plus any unexpected live peer, sorted alphabetically
by callsign. Roster fixtures keep their row while off air, so a two-second
...
- rows: physical roster plus live, every peer seen since bridge boot, or live now;
- place: all roster, the 111 art-site fixtures, four camp uplights, or three
  repair/offline canopy fixtures;
...
Open **Health** for the read-only, no-scroll fleet triage view. The 111-fixture
art-site roster fits as fixed squares on one 320x240 screen;
...
The embedded physical roster is generated by joining `ops/fleet/roster.csv`,
`ops/fleet/registry.csv`, and `ops/fleet/callsigns.csv`. Fleet keeps all 118
built fixtures. Health and RF expect only `site` scope, so the four camp uplights
and three repair canopy fixtures do not create false alerts. Any live ID outside
the complete physical roster is appended with a cyan border. The native test
wrapper regenerates the header and fails if the checked-in snapshot is stale.

The scoped-roster source and native/model checks pass. Per Ben's 2026-08-30
direction, this accumulated source change has not received an embedded build or
hardware flash; the installed T-Deck therefore still carries the preceding
unscoped roster until the next deliberate build/flash session.

The preceding unscoped Health baseline passed its native and embedded builds and
is flashed on `8EB508` in the 1,550,224-byte image documented above. Physical
acceptance of the new scoped source waits for the later accumulated build/flash.
