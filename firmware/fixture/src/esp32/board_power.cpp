...
// ADR 0051 battery-corroboration state: a plausible voltage alone is not
...
static void chargingGuardTick() {
  ...
  if (deepRecoveryBuild()) {
    ...
    gRecoveryEverDetected = true; // ADR 0051: BQ-qualified = corroborated
    ...
    gChargingEnabled = true;
    gLowVbatRecoveryChargeActive = true;
    gRecoveryEverDetected = true; // ADR 0051: presence test passed above
    gLowVbatRecoveryState = LOW_VBAT_RECOVERY_ACTIVE;
    pfSolarGuardInit(kTag, gMaintainV, true);
    done = true;
...
void readBatteryNow() {
  ...
  // ADR 0051 on-demand presence check: requested by the power policy when it
