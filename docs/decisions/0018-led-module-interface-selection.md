...
bench work. Net: IS31FL3741 13×9 is **ruled out** for the V2 battery build; the LED
...
- Efficiency numbers measured so far (e.g. HEX ~1.6× the PAR/mA of WS2812C NeoHEX) are
...
1. **IS31FL3741 13×9: ruled OUT for the V2 battery build.** Shared-bus brownout, well
   characterized and IS31-specific. Revisit only if the 13×9 *grid* form factor becomes a
...
**Original context:** the project wanted future-proof LED geometry — ideally 3×3, 5×5, or
...
1. **Adafruit IS31FL3741 13×9 matrix** as the primary no-solder PowerFeather/STEMMA-QT LED
   module.
2. **M5Stack NeoHEX** as the primary no-solder WS2812/GPIO geometry experiment.

FeatherS2 Neo and Atom Matrix remained integrated 5×5 fallback/optics boards.
