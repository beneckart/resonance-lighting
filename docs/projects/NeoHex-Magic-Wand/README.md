...
The canonical fleet registry marks `F40344` with role `magic_wand`.
`ops/bench/fleet_dashboard_ota.py` treats that role as protected: it refuses the
wand unless the operator supplies `--allow-special-target F40344`, and it still
refuses to mix the wand with any other OTA target. This is an acknowledgement,
not an artifact bypass; use only a dedicated Magic Wand binary.
