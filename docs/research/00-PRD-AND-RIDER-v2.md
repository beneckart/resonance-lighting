...
| A2 | Bamboo shell VISIBLE + lit | ✅ | 77cbfdb — warm lit material (#9c7a44) + key/rim lights; reads as a tree |
| A3 | Lanterns glow + bloom | ✅ | 77cbfdb — @react-three/postprocessing Bloom + HDR gain ×2.6 |
| A4 | Volumetric beams | ✅ | b383c80 — additive cones per fixture, beam-angle sized, reported color |
| A5 | Gobo/mandala projection | ✅ | c14 — ground + spotlight map=gobo-mandala.png, mandala on ground |
| A6 | Hero camera / night world / IES | ✅ | hero cam + ground/night (c14) + IES-ish beam photometrics (c31) |
| A7 | Multiple visualizers | ✅ | c16 — lanterns/orbs/wire render-style switch |
| B1 | FFT bands + mic/song/track | ✅ | b57501f |
| B2 | Beat/BPM/onset detection | ✅ | c13 — BeatTracker unit-tested: 124 & 90 BPM detected (math proven); headless~83 was audio-path not algorithm |
| B3 | Tested with REAL songs | 🟡 | b57501f — 124bpm track committed; headless tested, real-browser pending |
| B4 | Audio→modulation matrix | ✅ | b57501f — asym envelope followers + AGC + beat→flash |
| C1 | DJ sliders (crossfader/EQ/intensity/strobe) | ✅ | c18 — crossfade A↔B + EQ→zone + master + strobe, dj.ts unit-tested |
| C2 | MIDI controller (WebMIDI) | ✅ | c27 — ccToControl mapper unit-tested + guarded engine (CC→params, note→cue); device test pending |
| C3 | Guest-DJ scoped mode / AI-VJ | ✅ | c30 guest caps+lockout (unit-tested) + auto-VJ c19 |
| D1 | Pattern library (8: +spectrum/tricolor) | ✅ | bb0da17,c11 — full-spectrum + tricolor-dance |
| D2 | Random / auto-VJ modes | ✅ | c19 — shuffle-bag look switching on phrase (BPM-synced), unit-tested |
| D3 | Sequencer (7 modes) | ✅ | 2e91701 |
| D4 | Many sequences + element modes | ✅ | c23 — 7-mode sequencer + wind/ember/rain/beacon element modes |
| D5 | Cue/timeline/Show-Compiler | ✅ | c24 cues + c26 timeline (cue sequencing) + BPM→tempo; Show-Compiler = protocol param-export (c21) |
| E1 | Cortex (master brain) integration | ❌ | |
| E2 | ESP-NOW bridge / Protocol v1 | 🟡 | c21 — Protocol-v1 param ENCODER (params not pixels) + channel-pin + drive-real stub, unit-tested; real radio bridge = hardware-side later |
| F1 | LLM runs the command console | ✅ | c20 — runScript multi-cmd runner + docs/research/13 grammar spec; bridge to a real LLM is cortex-side |
| F2 | AI-VJ over live audio | ❌ | |
| H3 | Any-command console | ✅ | d9fdae6 |
| G* | Sensing/ops | 🟡 | c28 presence→ripple interactivity (sim) ✅; PIR/mmWave/weather/energy = hardware later |
