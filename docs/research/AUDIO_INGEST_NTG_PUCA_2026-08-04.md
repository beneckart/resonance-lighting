# Audio ingest -- VideoMic NTG -> PÚCA DSP setup notes (2026-08-04)
...
node extracts compact features (band energies, envelope, beat flag) and broadcasts
them over ESP-NOW at 30-60 Hz. This note records the hardware bought 2026-08-04 and
the setup/bench items so they aren't rediscovered on playa.
...
3. **Bench-verify NTG auto-power into the bias-less line-in.** The NTG auto-powers
   by sensing plug-in power from a camera/phone; the PÚCA line-in supplies none.
   Expected fine (the mic has manual power too), but confirm it powers on AND stays
   on feeding the PÚCA jack before it ships to playa. Find this quirk on the bench,
...
- **Tether/sandbag the Tripod 2** -- a light tripod wearing a furry windjammer is a
  sail.
- **PÚCA enclosure (print farm):** foam over the MEMS mic ports (same wind-as-bass
